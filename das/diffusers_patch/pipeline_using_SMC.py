...
from diffusers.utils import BaseOutput   # diffusers>=0.27

# from dataset.lexica import get_lexica_list

@dataclass
class SMCPipelineOutput(BaseOutput):
    ...
    latents_trace:           Optional[List[torch.Tensor]] = None
    log_w_trace:             Optional[List[torch.Tensor]] = None
    resample_indices:        Optional[List[torch.Tensor]] = None
    ess_trace:               Optional[torch.Tensor] = None
    scale_factor_trace:      Optional[torch.Tensor] = None
    rewards_trace:           Optional[torch.Tensor] = None
    manifold_deviation:      Optional[torch.Tensor] = None
    log_prob_diffusion_trace:Optional[torch.Tensor] = None
...
def pipeline_using_smc(
    ...
    # SMC parameters
    num_particles: int = 4,
    batch_p: int = 1, # number of particles to run parallely, NOT USING
    ...
    kl_coeff: float = 0.005,
    verbose: bool = False # True for debugging SMC procedure
):
    # hard-coded parameters for aesthetic score (pa reward) only
    ...
    prompt_embeds = self._encode_prompt(
        prompt,
        device,
        num_particles, # num_images_per_prompt
    ...
    def _decode(latents):
        image = self.vae.decode(
            latents.to(torch.float16) / self.vae.config.scaling_factor, return_dict=False
        )[0]
    ...
    def _calc_guidance():
        if (i >= start):
            ...
            # nonlocal log_w, log_Z, log_prob_diffusion, log_prob_proposal, log_twist_func, approx_guidance, rewards, noise_pred, guidance
            with torch.enable_grad():
                tmp_latents = latents[:].detach().clone().to(torch.float32).requires_grad_(True)
                ...
                # Calculate rewards
                decoded_images = _decode(tmp_pred_original_sample)
                images = pipe_image_process(decoded_images)
                image_list = [image for image in images]
                veri_input = self.verifier.prepare_inputs(image_list, [ori_prompt] * len(image_list) )
                
                # veri_input = self.verifier.prepare_inputs(images, prompt)
                
                tmp_rewards_info = reward_fn(veri_input, 'tensor', prompt_idx)
                
      
                rewards_tensor = torch.stack(
                    [torch.as_tensor(info["reward"]) for info in tmp_rewards_info]     # → shape (N, …)
                )
                tmp_rewards = rewards_tensor #.mean()
                ...
                pred_original_sample[:] = tmp_pred_original_sample.detach().clone()
                rewards[:] = tmp_rewards.detach().clone()
                

                noise_pred[:] = tmp_noise_pred.detach().clone()
                guidance[:] = tmp_guidance.detach().clone()

                log_twist_func[:] = tmp_log_twist_func.detach().clone()
                approx_guidance[:] = tmp_approx_guidance.clone()
            ...
            tmp_latents = latents[:].detach().requires_grad_(True)
            with torch.no_grad():
                tmp_latents = latents[:].detach()
                tmp_noise_pred, tmp_guidance = _pred_noise(tmp_latents, t)
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            _calc_guidance()
            rewards_trace.append(rewards.view(-1, num_particles).max(dim=1)[0].cpu()) # TODO: check if max is right
        ...
        #veri
        images = pipe_image_process(tmp_image)
        image_list = [image for image in images]
        veri_input = self.verifier.prepare_inputs(image_list, [ori_prompt] * len(image_list) )
                
        tmp_rewards_info = reward_fn(veri_input, 'tensor', prompt_idx)
        rewards_tensor = torch.stack(
                    [torch.as_tensor(info["reward"]) for info in tmp_rewards_info]     
                )
       
        
        tmp_rewards = rewards_tensor #.mean()
        # tmp_rewards = reward_fn(tmp_image, 'tensor', prompt_idx).detach().to(torch.float32)
        rewards[:] = tmp_rewards
    ...
    manifold_deviation_trace = manifold_deviation_trace.cpu()
    log_prob_diffusion_trace = - log_prob_diffusion_trace.cpu() / 4 / 64 / 64 / math.log(2) # bits per dimension
    images_list = [img for img in (output_images if isinstance(output_images, list) else output_images)]

    return SMCPipelineOutput(
        images                   = images_list,
        log_weights              = log_w,
        normalized_weights       = normalized_w,
        latents_trace            = all_latents,
        log_w_trace              = all_log_w,
        resample_indices         = all_resample_indices,
        ess_trace                = ess_trace,
        scale_factor_trace       = scale_factor_trace,
        rewards_trace            = rewards_trace,
        manifold_deviation       = manifold_deviation_trace,
